...
# Optional: an Application API key from a Pterodactyl Panel (Account Settings ->
# API Credentials -> Create New, with Nodes: Read/Write + Locations: Read/Write).
# Required only for !wings to auto-create/link nodes. Not needed for !pterodactyl.
PTERODACTYL_APP_API_KEY = os.getenv('PTERODACTYL_APP_API_KEY', '')
...
# Global VPS slot cap — only used for the bot's rotating status display now.
# Self-service !deploy has been replaced by the credits/!buy system below.
#   DEPLOY_SLOT=50   (optional — total VPS slot cap shown in bot presence, 0 = unlimited)
...
# ── VPS Plans (credits) ──────────────────────────────────────────────
# price = cost in credits. Adjust freely — this is just a starting point
# (1 CPU core + 20GB disk added per 4GB RAM step, 100 credits per GB).
PLANS = {
    "4gb":  {"ram": 4,  "cpu": 1, "disk": 20,  "price": 400},
    "8gb":  {"ram": 8,  "cpu": 2, "disk": 40,  "price": 800},
    "12gb": {"ram": 12, "cpu": 3, "disk": 60,  "price": 1200},
    "16gb": {"ram": 16, "cpu": 4, "disk": 80,  "price": 1600},
}
...
def init_db():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS pterodactyl_installs (
        container_name TEXT PRIMARY KEY,
        panel_domain TEXT,
        panel_admin_username TEXT,
        panel_admin_email TEXT,
        panel_installed_at TEXT,
        wings_domain TEXT,
        wings_port INTEGER,
        wings_scheme TEXT,
        wings_node_id INTEGER,
        wings_installed_at TEXT
    )''')
    conn.commit()
    conn.close()

# ── Credit / coupon helpers ─────────────────────────────────────────
def get_credits(user_id: str) -> int:
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT balance FROM credits WHERE user_id = ?', (str(user_id),))
    row = cur.fetchone()
    conn.close()
...
def add_credits(user_id: str, amount: int):
    conn = get_db()
    cur = conn.cursor()
    cur.execute('INSERT INTO credits (user_id, balance) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET balance = balance + ?',
...
def deduct_credits(user_id: str, amount: int) -> bool:
    ...
    if balance < amount:
        ...
        return False
    cur.execute('INSERT INTO credits (user_id, balance) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET balance = balance - ?',
                (str(user_id), balance - amount, amount))
    conn.commit()
    conn.close()
    return True

def create_coupon(code: str, amount: int, created_by: str) -> bool:
    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute('INSERT INTO coupons (code, amount, created_by, created_at) VALUES (?, ?, ?, ?)',
                    (code, amount, str(created_by), datetime.now().isoformat()))
        conn.commit()
        return True
...
def redeem_coupon(code: str, user_id: str):
    """Returns (success: bool, message_or_amount)."""
    conn = get_db()
    cur = conn.cursor()
    ...
    if row['redeemed_by']:
        conn.close()
        return False, "That coupon has already been redeemed."
    cur.execute('UPDATE coupons SET redeemed_by = ?, redeemed_at = ? WHERE code = ?',
                (str(user_id), datetime.now().isoformat(), code))
    conn.commit()
    amount = row['amount']
    conn.close()
...
# ── Pterodactyl / Wings install tracking ────────────────────────────
def get_pterodactyl_install(container_name: str):
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT * FROM pterodactyl_installs WHERE container_name = ?', (container_name,))
    row = cur.fetchone()
    conn.close()
    return dict(row) if row else None

def save_panel_install(container_name: str, domain: str, username: str, email: str):
    conn = get_db()
    cur = conn.cursor()
    cur.execute('''INSERT INTO pterodactyl_installs (container_name, panel_domain, panel_admin_username, panel_admin_email, panel_installed_at)
                   VALUES (?, ?, ?, ?, ?)
                   ON CONFLICT(container_name) DO UPDATE SET
                     panel_domain=excluded.panel_domain,
                     panel_admin_username=excluded.panel_admin_username,
                     panel_admin_email=excluded.panel_admin_email,
                     panel_installed_at=excluded.panel_installed_at''',
                (container_name, domain, username, email, datetime.now().isoformat()))
    conn.commit()
    conn.close()

def save_wings_install(container_name: str, domain: str, port: int, scheme: str, node_id: int):
    conn = get_db()
    cur = conn.cursor()
    cur.execute('''UPDATE pterodactyl_installs SET wings_domain=?, wings_port=?, wings_scheme=?, wings_node_id=?, wings_installed_at=?
                   WHERE container_name=?''',
                (domain, port, scheme, node_id, datetime.now().isoformat(), container_name))
    conn.commit()
    conn.close()
...
# ── Fixed (Cloudflare-compatible) port forwards for Panel/Wings ────
CLOUDFLARE_HTTPS_PORTS = [443, 2053, 2083, 2087, 2096, 8443]
CLOUDFLARE_HTTP_PORTS = [80, 2052, 2082, 2086, 2095, 8080]

def get_used_host_ports_for_node(node_id: int) -> set:
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT host_port FROM port_forwards WHERE vps_container IN (SELECT container_name FROM vps WHERE node_id = ?)', (node_id,))
...
def pick_fixed_port(node_id: int, candidates: List[int]) -> Optional[int]:
    used = get_used_host_ports_for_node(node_id)
    for port in candidates:
        if port not in used:
            return port
    return None

async def create_fixed_port_forward(user_id: str, container: str, container_port: int, node_id: int, host_port: int) -> Optional[int]:
    """Like create_port_forward but with a caller-chosen (not random) host port. TCP only."""
    try:
        await execute_lxc(container, f"config device add {container} tcp_proxy_{host_port} proxy listen=tcp:0.0.0.0:{host_port} connect=tcp:127.0.0.1:{container_port}", node_id=node_id)
        conn = get_db()
        cur = conn.cursor()
        cur.execute('INSERT INTO port_forwards (user_id, vps_container, vps_port, host_port, created_at) VALUES (?, ?, ?, ?, ?)',
                    (user_id, container, container_port, host_port, datetime.now().isoformat()))
        conn.commit()
        conn.close()
        return host_port
    except Exception as e:
        logger.error(f"Failed to create fixed port forward: {e}")
        return None
...
class NoDMsAllowed(commands.CheckFailure):
    pass

@bot.check
async def globally_block_dms(ctx):
    if ctx.guild is None:
        raise NoDMsAllowed("DMs Commands Disabled. Please use in server.")
    return True
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        return
    elif isinstance(error, NoDMsAllowed):
        await ctx.send(embed=create_warning_embed(
            "🔒 Server-Only Command",
            "Commands can't be used in DMs — please head back to the server and run this command there instead!"
        ))
...
class HelpView(discord.ui.View):
        ...
        self.command_categories = {
            "user": {
                ...
                "commands": [
                    ...
                    (f"{PREFIX}setpwd <container> <password>", "Change your VPS's root password"),
                    (f"{PREFIX}autofix <container>", "Attempt to auto-fix common VPS issues"),
                    (f"{PREFIX}pterodactyl <container>", "Install a Pterodactyl Panel on your VPS"),
                    (f"{PREFIX}wings <container>", "Install the Wings daemon on your VPS")
                ]
            },
            "shop": {
                "name": "🛒 Credits & Plans",
                "commands": [
                    (f"{PREFIX}plans", "View available VPS plans and prices"),
                    (f"{PREFIX}balance", "Check your credit balance"),
                    (f"{PREFIX}buy <plan>", "Purchase a VPS plan with your credits"),
                    (f"{PREFIX}redeem <code>", "Redeem a coupon code for credits"),
                    (f"{PREFIX}addcoupon <code> <amount>", "Create a redeemable coupon (Admin only)")
                ]
            },
            "vps": {
    ...
    def update_select(self):
        ...
        basic_categories = ["user", "shop", "vps", "ports", "system", "bot"]
...
# ═══════════════════════════════════════════════════════════════════
# Credits, Coupons, Plans & !buy — VPS purchasing system
# ═══════════════════════════════════════════════════════════════════

_deploying_users: set = set()


def find_user_vps(user_id: str, container_name: str):
    """Return the vps dict if container_name belongs to user_id, else None."""
    for vps in vps_data.get(str(user_id), []):
        if vps['container_name'] == container_name:
            return vps
    return None
...
async def balance_cmd(ctx):
    ...
    embed = create_info_embed("💳 Your Balance", f"You currently have **{bal}** credits.")
    await ctx.send(embed=embed)
...
async def plans_cmd(ctx):
    ...
    embed = create_info_embed("📦 VPS Plans", "Use `!buy <plan>` to purchase a plan with your credits.")
    for key, plan in PLANS.items():
        add_field(
            embed,
            f"{plan['ram']}GB Plan (`{key}`)",
            f"**RAM:** {plan['ram']}GB | **CPU:** {plan['cpu']} Cores | **Disk:** {plan['disk']}GB\n**Price:** {plan['price']} credits",
            False
        )
    add_field(embed, "🛠️ Custom Plan", "Need something bigger or different? Contact an admin to arrange a custom plan.", False)
    await ctx.send(embed=embed)


@bot.command(name='addcoupon')
@is_admin()
async def addcoupon_cmd(ctx, code: str, amount: int):
    """Create a redeemable coupon worth `amount` credits (admin only)."""
    if amount <= 0:
        await ctx.send(embed=create_error_embed("Invalid Amount", "Coupon amount must be greater than 0."))
        return
    code = code.strip().upper()
    if create_coupon(code, amount, ctx.author.id):
        embed = create_success_embed("🎟️ Coupon Created", f"Code `{code}` created for **{amount}** credits.")
        await ctx.send(embed=embed)
    else:
        await ctx.send(embed=create_error_embed("Coupon Exists", f"A coupon with code `{code}` already exists."))


@bot.command(name='redeem')
async def redeem_cmd(ctx, code: str = None):
    """Redeem a coupon code for credits."""
    if not code:
        await ctx.send(embed=create_error_embed("Usage", f"`{PREFIX}redeem <code>`"))
        return
    code = code.strip().upper()
    success, result = redeem_coupon(code, ctx.author.id)
    if success:
        embed = create_success_embed("🎉 Coupon Redeemed", f"You received **{result}** credits!")
        add_field(embed, "New Balance", f"{get_credits(ctx.author.id)} credits", False)
        await ctx.send(embed=embed)
    else:
        await ctx.send(embed=create_error_embed("Redeem Failed", result))


class BuyOSView(discord.ui.View):
    """OS selection dropdown shown after the user confirms a !buy purchase."""
    def __init__(self, user: discord.Member, ctx, node_id: int, plan_key: str):
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        try:
            await _do_plan_deploy(self.ctx, self.user, os_value, self.node_id, self.plan_key)
        finally:
...
class BuyConfirmView(discord.ui.View):
    """Confirm/Cancel buttons shown by !buy with resource info."""
    def __init__(self, user: discord.Member, ctx, node_id: int, plan_key: str):
        super().__init__(timeout=120)
        self.user = user
        self.ctx = ctx
    ...
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        os_view = BuyOSView(self.user, self.ctx, self.node_id, self.plan_key)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for child in self.children:
            child.disabled = True
        await interaction.response.edit_message(view=self)
        await interaction.followup.send(embed=create_info_embed("Cancelled", "Purchase cancelled. Your credits were not charged."))
        _deploying_users.discard(str(self.user.id))
...
async def _do_plan_deploy(ctx, user: discord.Member, os_version: str, node_id: int, plan_key: str):
    """Core deploy logic for !buy. Charges credits only after a successful deploy."""
    user_id = str(user.id)
    plan = PLANS[plan_key]
    try:
        ...
        await execute_lxc(container_name, f"config set {container_name} limits.memory {ram_mb}MB", node_id=node_id)
        await execute_lxc(container_name, f"config set {container_name} limits.cpu {plan['cpu']}", node_id=node_id)
        await execute_lxc(container_name, f"config device set {container_name} root size={plan['disk']}GB", node_id=node_id)
        await apply_lxc_config(container_name, node_id)
        await execute_lxc(container_name, f"start {container_name}", node_id=node_id)
        await apply_internal_permissions(container_name, node_id)
        ...
        try:
            ...
            logger.warning(f"Buy SSH port forward failed: {e}")
        ...
        config_str = f"{plan['ram']}GB RAM / {plan['cpu']} CPU / {plan['disk']}GB Disk"
        ...
        # Charge credits only now that the VPS actually exists
        deduct_credits(user_id, plan["price"])

        if ctx.guild:
            vps_role = await get_or_create_vps_role(ctx.guild)
            if vps_role:
                try:
                    await user.add_roles(vps_role, reason="VPS purchased via !buy")
                except discord.Forbidden:
                    pass

        dm_embed = create_success_embed("🎉 VPS Deployed!", "Your purchased VPS is ready!")
        ...
        pub_embed.add_field(name="Charged", value=f"{plan['price']} credits", inline=True)
        if ssh_host_port:
            pub_embed.add_field(
                name="🌐 SSH",
        ...
        logger.error(f"!buy deploy failed for {user}: {e}")
        await ctx.send(embed=create_error_embed("Deploy Failed", f"{str(e)}\n\nYou were not charged."))
        _deploying_users.discard(str(user.id))


@bot.command(name='buy')
async def buy_cmd(ctx, plan_key: str = None):
    """Purchase a VPS plan using your credits."""
    user_id = str(ctx.author.id)

    if not plan_key or plan_key.lower() not in PLANS:
        plan_list = ", ".join(f"`{k}`" for k in PLANS.keys())
        await ctx.send(embed=create_error_embed(
            "Usage", f"`{PREFIX}buy <plan>`\nAvailable plans: {plan_list}\nSee `{PREFIX}plans` for details, or contact an admin for a custom plan."
        ))
        return
    ...
    if balance < plan["price"]:
        await ctx.send(embed=create_error_embed(
            ...
            f"The **{plan['ram']}GB** plan costs **{plan['price']}** credits, but you only have **{balance}**.\n"
            f"Use `{PREFIX}redeem <code>` if you have a coupon."
        ))
        return
    ...
    embed = create_info_embed(
        ...
        f"> **Plan:** `{plan_key}`\n"
        f"> **RAM:** `{plan['ram']}GB`\n"
        f"> **CPU:** `{plan['cpu']}` Cores\n"
        f"> **Disk:** `{plan['disk']}GB`\n"
        f"> **Price:** `{plan['price']}` credits\n"
        f"> **Your Balance:** `{balance}` credits\n\n"
        f"Click **✅ Confirm Purchase** to choose your OS and deploy!"
    )
    view = BuyConfirmView(ctx.author, ctx, node_id, plan_key)
    await ctx.send(embed=embed, view=view)


# ═══════════════════════════════════════════════════════════════════
# !setpwd — change your own VPS's root password
# ═══════════════════════════════════════════════════════════════════

@bot.command(name='setpwd')
async def setpwd_cmd(ctx, container_name: str = None, password: str = None):
    """Change the root password of a VPS you own."""
    if not container_name or not password:
        await ctx.send(embed=create_error_embed("Usage", f"`{PREFIX}setpwd <container_name> <new_password>`"))
        return

    vps = find_user_vps(ctx.author.id, container_name)
    if not vps:
        await ctx.send(embed=create_error_embed("Not Found", "You don't own a VPS with that container name."))
        return

    if len(password) < 6:
        await ctx.send(embed=create_error_embed("Weak Password", "Please choose a password with at least 6 characters."))
        return

    node_id = vps.get('node_id', 1)
    msg = await ctx.send(embed=create_info_embed("🔄 Updating Password", "Please wait…"))
    success, result = await configure_ssh(container_name, node_id, password)
    if success:
        embed = create_success_embed("🔐 Password Updated", f"The root password for `{container_name}` has been changed.")
        try:
            await msg.edit(embed=embed)
        except discord.HTTPException:
            await ctx.send(embed=embed)
        try:
            dm_embed = create_success_embed("🔐 VPS Password Changed", f"Your VPS `{container_name}` password was updated.")
            add_field(dm_embed, "New Password", f"`{password}`", False)
            await ctx.author.send(embed=dm_embed)
        except discord.Forbidden:
            pass
    else:
        await ctx.send(embed=create_error_embed("Update Failed", f"Could not update the password: {result}"))


# ═══════════════════════════════════════════════════════════════════
# !autofix — best-effort automatic fixes for common VPS issues
# ═══════════════════════════════════════════════════════════════════

AUTOFIX_STEPS = [
    ("Fixing broken packages", "dpkg --configure -a 2>/dev/null; apt-get -f install -y 2>/dev/null; true"),
    ("Refreshing package lists", "apt-get update -y 2>/dev/null; true"),
    ("Restarting networking", "systemctl restart networking 2>/dev/null || service networking restart 2>/dev/null || true"),
    ("Resetting DNS resolution", "echo 'nameserver 8.8.8.8' > /etc/resolv.conf 2>/dev/null; echo 'nameserver 1.1.1.1' >> /etc/resolv.conf 2>/dev/null; true"),
    ("Restarting SSH service", "systemctl restart ssh 2>/dev/null || service ssh restart 2>/dev/null || /etc/init.d/ssh restart 2>/dev/null || true"),
    ("Clearing package cache / old logs", "apt-get clean 2>/dev/null; journalctl --vacuum-time=3d 2>/dev/null; true"),
]


@bot.command(name='autofix')
async def autofix_cmd(ctx, container_name: str = None):
    """Attempt to automatically fix common issues on a VPS you own."""
    if not container_name:
        await ctx.send(embed=create_error_embed("Usage", f"`{PREFIX}autofix <container_name>`"))
        return
    ...
    node_id = vps.get('node_id', 1)
    status_embed = create_info_embed("🩺 Running Autofix", f"Checking `{container_name}` for common issues…")
    msg = await ctx.send(embed=status_embed)

    results = []
    any_failed = False
    for label, cmd in AUTOFIX_STEPS:
        try:
            await execute_lxc(container_name, f'exec {container_name} -- bash -c "{cmd}"', node_id=node_id)
            results.append(f"✅ {label}")
        except Exception as e:
            results.append(f"⚠️ {label} — {str(e)[:80]}")
            any_failed = True

    embed = create_success_embed("🩺 Autofix Complete", f"Ran common fixes on `{container_name}`:")
    add_field(embed, "Steps", "\n".join(results), False)

    if any_failed:
        add_field(embed, "Still having issues?",
                  "Some steps couldn't be confirmed. This has been forwarded to an admin — "
                  "if the problem persists, please open a ticket.", False)
        try:
            admin_user = await bot.fetch_user(MAIN_ADMIN_ID)
            admin_embed = create_warning_embed(
                "🩺 Autofix Escalation",
                f"{ctx.author} (`{ctx.author.id}`) ran `!autofix` on `{container_name}` and some steps failed.\n"
                f"Please check the VPS manually."
            )
            await admin_user.send(embed=admin_embed)
        except Exception as e:
            logger.warning(f"Could not DM admin about autofix escalation: {e}")
    else:
        add_field(embed, "Still having issues?",
                  "If your problem isn't resolved, please open a ticket and an admin will take a look.", False)
...
# ═══════════════════════════════════════════════════════════════════
# !pterodactyl / !wings — Game panel automation
# ═══════════════════════════════════════════════════════════════════
# Notes / known limitations (see CHANGES.md for the full write-up):
#  - Panel + Wings are installed inside the SAME container the user owns.
#  - SSL for the Panel is real Let's Encrypt (certbot). Wings itself runs
#    plain HTTP internally on the container side and is exposed on
#    whichever host port you pick — if you pick a 443/8443-style port,
#    set that domain's Cloudflare SSL mode to "Flexible" (edge HTTPS,
#    plain HTTP to origin), since Wings isn't terminating TLS itself here.
#  - Linking Wings to the Panel requires PTERODACTYL_APP_API_KEY to be
#    set in .env (create it once via Panel -> Account -> API Credentials,
#    with Nodes: Read/Write + Locations: Read/Write).

PANEL_INSTALL_SCRIPT = r'''
apt-get update -y
apt-get install -y software-properties-common curl apt-transport-https ca-certificates gnupg lsb-release unzip git tar cron

curl -sSLo /tmp/sury.sh https://packages.sury.org/php/README.txt 2>/dev/null || true
if [ -f /tmp/sury.sh ]; then bash /tmp/sury.sh || true; fi
apt-get update -y
...
cat > /etc/systemd/system/pteroq.service << 'EOFSERVICE'
[Unit]
...
certbot --nginx -d "${DOMAIN}" --non-interactive --agree-tos -m "${ADMIN_EMAIL}" --redirect || echo "CERTBOT_FAILED_CONTINUING"

systemctl restart nginx
systemctl restart php8.1-fpm
echo "PANEL_INSTALL_DONE"
'''

WINGS_INSTALL_SCRIPT = r'''
apt-get update -y
...
wings configure --panel-url "${PANEL_URL}" --token "${APP_API_KEY}" --node "${NODE_ID}"

systemctl enable --now wings
sleep 5
if systemctl is-active --quiet wings; then echo "WINGS_RUNNING"; else echo "WINGS_FAILED_TO_START"; fi
'''

BLUEPRINT_INSTALL_SCRIPT = r'''
cd /var/www/pterodactyl
curl -L -o blueprint.zip https://github.com/BlueprintFramework/framework/releases/latest/download/blueprint.zip
unzip -o blueprint.zip -d .
chmod +x blueprint.sh
bash blueprint.sh --install --no-interaction || echo "BLUEPRINT_FAILED"
echo "BLUEPRINT_DONE"
'''


async def run_container_script(container_name: str, node_id: int, var_lines: str, static_script: str, timeout: int = 1800):
    """Base64-encode a bash script (with a few shell var assignments prepended) and run it in the container."""
    full_script = "#!/bin/bash\nset -e\nexport DEBIAN_FRONTEND=noninteractive\n" + var_lines + static_script
    encoded = base64.b64encode(full_script.encode()).decode()
    cmd = f'exec {container_name} -- bash -c "echo {encoded} | base64 -d > /root/_autosetup.sh && bash /root/_autosetup.sh; rm -f /root/_autosetup.sh"'
    return await execute_lxc(container_name, cmd, node_id=node_id, timeout=timeout)


# ── !pterodactyl ─────────────────────────────────────────────────────

class PterodactylSetupModal(discord.ui.Modal, title="Pterodactyl Panel Setup"):
    domain = discord.ui.TextInput(label="Domain (e.g. panel.example.com)", placeholder="panel.example.com", max_length=255)
    admin_username = discord.ui.TextInput(label="Admin Username", max_length=32)
    admin_email = discord.ui.TextInput(label="Admin Email", max_length=255)
    admin_password = discord.ui.TextInput(label="Admin Password (8+ chars, mixed case + number)", max_length=64)

    def __init__(self, ctx, container_name: str, node_id: int):
        super().__init__()
        self.ctx = ctx
        self.container_name = container_name
        self.node_id = node_id
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            embed=create_info_embed(
                ...
                f"Setting up `{self.container_name}` with domain `{str(self.domain).strip()}`.\n"
                f"This takes **5-10 minutes** — you'll get a DM when it's done."
            ),
            ephemeral=True
        )
        asyncio.create_task(install_pterodactyl_panel(
            self.ctx, interaction.user, self.container_name, self.node_id,
            str(self.domain).strip().lower(), str(self.admin_username).strip(),
            str(self.admin_email).strip(), str(self.admin_password)
        ))


class PterodactylStartView(discord.ui.View):
    def __init__(self, user: discord.Member, ctx, container_name: str, node_id: int):
    ...
    @discord.ui.button(label="🦅 Setup Panel", style=discord.ButtonStyle.success)
    async def setup(self, interaction: discord.Interaction, button: discord.ui.Button):
        if str(interaction.user.id) != str(self.user.id):
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only you can start this setup."), ephemeral=True)
            return
        await interaction.response.send_modal(PterodactylSetupModal(self.ctx, self.container_name, self.node_id))


async def install_pterodactyl_panel(ctx, user: discord.Member, container_name: str, node_id: int,
                                     domain: str, admin_username: str, admin_email: str, admin_password: str):
    channel = ctx.channel
    db_pass = generate_strong_password(20)
    var_lines = (
        f"DOMAIN={shlex.quote(domain)}\n"
        f"ADMIN_USER={shlex.quote(admin_username)}\n"
        f"ADMIN_EMAIL={shlex.quote(admin_email)}\n"
        f"ADMIN_PASS={shlex.quote(admin_password)}\n"
        f"DB_PASS={shlex.quote(db_pass)}\n"
    )
    try:
        output = await run_container_script(container_name, node_id, var_lines, PANEL_INSTALL_SCRIPT, timeout=1800)
        output_str = str(output) if output else ""
        certbot_failed = "CERTBOT_FAILED_CONTINUING" in output_str

        # Forward host ports: 80 (ACME/redirect) + a Cloudflare-safe HTTPS port
        https_port = pick_fixed_port(node_id, CLOUDFLARE_HTTPS_PORTS)
        http_port = pick_fixed_port(node_id, CLOUDFLARE_HTTP_PORTS)
        if https_port:
            await create_fixed_port_forward(str(user.id), container_name, 443, node_id, https_port)
        if http_port:
            await create_fixed_port_forward(str(user.id), container_name, 80, node_id, http_port)

        save_panel_install(container_name, domain, admin_username, admin_email)

        embed = create_success_embed("🦅 Pterodactyl Panel Installed!", f"Panel is live on `{container_name}`.")
        add_field(embed, "Domain", f"`{domain}`", False)
        add_field(embed, "Public Access", (
            f"HTTPS host port: `{https_port}`\n" if https_port else "⚠️ Could not allocate an HTTPS host port.\n"
        ) + (
            f"HTTP host port (for cert renewal): `{http_port}`" if http_port else "⚠️ Could not allocate an HTTP host port."
        ), False)
        if certbot_failed:
            add_field(embed, "⚠️ SSL Certificate", "Let's Encrypt failed — make sure the domain's DNS already points at this server before running certbot again manually inside the container.", False)
        add_field(embed, "Admin Login", f"**Username:** `{admin_username}`\n**Email:** `{admin_email}`\n**Password:** `{admin_password}`", False)
        add_field(embed, "Next Step", f"Run `{PREFIX}wings {container_name}` to install the game server daemon on this same VPS.", False)
        try:
            await user.send(embed=embed)
        except discord.Forbidden:
            pass
        await channel.send(embed=create_success_embed("✅ Panel Installed", f"{user.mention} your Pterodactyl Panel is ready! Check your DMs for login details."))
    except Exception as e:
        logger.error(f"Pterodactyl panel install failed for {container_name}: {e}")
        await channel.send(embed=create_error_embed("Panel Install Failed", f"{str(e)[:1500]}"))


@bot.command(name='pterodactyl')
async def pterodactyl_cmd(ctx, container_name: str = None):
    """Install a Pterodactyl Panel on a VPS you own."""
    if not container_name:
        await ctx.send(embed=create_error_embed("Usage", f"`{PREFIX}pterodactyl <container_name>`"))
        return
    vps = find_user_vps(ctx.author.id, container_name)
    if not vps:
        await ctx.send(embed=create_error_embed("Not Found", "You don't own a VPS with that container name."))
        return
    existing = get_pterodactyl_install(container_name)
    if existing and existing.get('panel_installed_at'):
        await ctx.send(embed=create_error_embed("Already Installed", f"Pterodactyl Panel is already installed on `{container_name}` (domain: `{existing['panel_domain']}`)."))
        return
    embed = create_info_embed(
        "🦅 Pterodactyl Panel Setup",
        "Click below and fill in your domain + admin account details.\n\n"
        "⚠️ Make sure your domain's DNS already points at this server before starting, "
        "so the SSL certificate can be issued."
    )
    view = PterodactylStartView(ctx.author, ctx, container_name, vps.get('node_id', 1))
    await ctx.send(embed=embed, view=view)


# ── !wings ───────────────────────────────────────────────────────────

class WingsSetupModal(discord.ui.Modal, title="Wings Daemon Setup"):
    domain = discord.ui.TextInput(label="Wings Domain (e.g. node.example.com)", placeholder="node.example.com", max_length=255)

    def __init__(self, ctx, container_name: str, node_id: int, port: int, scheme: str):
        super().__init__()
        self.ctx = ctx
        self.container_name = container_name
        self.node_id = node_id
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            embed=create_info_embed(
                "🐦 Installing Wings",
                f"Setting up Wings on `{self.container_name}`. This takes a few minutes — you'll get a DM when it's done."
            ),
            ephemeral=True
        )
        asyncio.create_task(install_wings(
            self.ctx, interaction.user, self.container_name, self.node_id,
            str(self.domain).strip().lower(), self.port, self.scheme
        ))


class WingsPortChoiceView(discord.ui.View):
    def __init__(self, user: discord.Member, ctx, container_name: str, node_id: int):
        super().__init__(timeout=180)
        self.user = user
        self.ctx = ctx
    ...
    async def _check(self, interaction: discord.Interaction) -> bool:
        if str(interaction.user.id) != str(self.user.id):
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only you can pick this."), ephemeral=True)
            return False
        return True

    @discord.ui.button(label="8080 (HTTP)", style=discord.ButtonStyle.secondary)
    async def opt_8080(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await self._check(interaction):
            return
        await interaction.response.send_modal(WingsSetupModal(self.ctx, self.container_name, self.node_id, 8080, "http"))

    @discord.ui.button(label="443 (HTTPS via Cloudflare)", style=discord.ButtonStyle.primary)
    async def opt_443(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await self._check(interaction):
            return
        await interaction.response.send_modal(WingsSetupModal(self.ctx, self.container_name, self.node_id, 443, "https"))

    @discord.ui.button(label="8443 (HTTPS via Cloudflare)", style=discord.ButtonStyle.primary)
    async def opt_8443(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await self._check(interaction):
            return
        await interaction.response.send_modal(WingsSetupModal(self.ctx, self.container_name, self.node_id, 8443, "https"))


async def install_wings(ctx, user: discord.Member, container_name: str, node_id: int, domain: str, requested_port: int, scheme: str):
    channel = ctx.channel

    if not PTERODACTYL_APP_API_KEY:
        await channel.send(embed=create_error_embed(
            "Wings Setup Blocked",
            "No `PTERODACTYL_APP_API_KEY` is configured in `.env`.\n\n"
            "Ask an admin to create one: in the Panel go to **Account Settings → API Credentials → Create New**, "
            "grant **Nodes: Read/Write** and **Locations: Read/Write**, then add it to `.env` as "
            "`PTERODACTYL_APP_API_KEY=ptla_...` and restart the bot."
        ))
        return

    install = get_pterodactyl_install(container_name)
    if not install or not install.get('panel_installed_at'):
        await channel.send(embed=create_error_embed("Panel Required", f"Run `{PREFIX}pterodactyl {container_name}` first — Wings needs a Panel to connect to."))
        return
    panel_domain = install['panel_domain']
    panel_url = f"https://{panel_domain}"
    headers = {
        "Authorization": f"Bearer {PTERODACTYL_APP_API_KEY}",
        "Accept": "application/vnd.pterodactyl.v1+json",
        "Content-Type": "application/json",
    }

    candidates = CLOUDFLARE_HTTPS_PORTS if scheme == "https" else CLOUDFLARE_HTTP_PORTS
    # If their preferred port is free, use it; otherwise fall back to the next free one in that list.
    host_port = requested_port if requested_port not in get_used_host_ports_for_node(node_id) else pick_fixed_port(node_id, candidates)
    if not host_port:
        await channel.send(embed=create_error_embed("No Ports Available", "All Cloudflare-compatible ports on this node are already in use. Contact an admin."))
        return

    vps = find_user_vps(user.id, container_name) or {}
    ram_mb = int(str(vps.get('ram', '4GB')).replace('GB', '').strip() or 4) * 1024
    disk_mb = int(str(vps.get('storage', '20GB')).replace('GB', '').strip() or 20) * 1024

    try:
        # 1. Create a Location for this node (one per container, kept simple/self-contained)
        short_code = sanitize_username_for_container(container_name)[:16] or "auto"
        loc_resp = requests.post(f"{panel_url}/api/application/locations", headers=headers, json={
            "short": short_code, "long": f"Auto-created for {container_name}"
        }, timeout=30)
        location_id = None
        if loc_resp.status_code in (200, 201):
            location_id = loc_resp.json()["attributes"]["id"]
        else:
            # Might already exist from a retry — look it up
            list_resp = requests.get(f"{panel_url}/api/application/locations", headers=headers, timeout=30)
            if list_resp.status_code == 200:
                for entry in list_resp.json().get("data", []):
                    if entry["attributes"]["short"] == short_code:
                        location_id = entry["attributes"]["id"]
                        break
        if not location_id:
            await channel.send(embed=create_error_embed("Wings Setup Failed", f"Could not create/find a Panel location.\n{loc_resp.text[:500]}"))
            return

        # 2. Create the Node
        node_resp = requests.post(f"{panel_url}/api/application/nodes", headers=headers, json={
            "name": container_name,
            "location_id": location_id,
            "fqdn": domain,
            "scheme": "http",  # Wings itself serves plain HTTP internally in this setup — see command notes
            "behind_proxy": False,
            "memory": ram_mb,
        ...
        if node_resp.status_code not in (200, 201):
            await channel.send(embed=create_error_embed("Wings Setup Failed", f"Could not create the Panel node.\n{node_resp.text[:500]}"))
            return
        node_id_ptero = node_resp.json()["attributes"]["id"]

        # 3. Install Wings + link it to the node we just created
        var_lines = (
            f"PANEL_URL={shlex.quote(panel_url)}\n"
            f"APP_API_KEY={shlex.quote(PTERODACTYL_APP_API_KEY)}\n"
            f"NODE_ID={shlex.quote(str(node_id_ptero))}\n"
        )
        output = await run_container_script(container_name, node_id, var_lines, WINGS_INSTALL_SCRIPT, timeout=900)
        output_str = str(output) if output else ""
        wings_ok = "WINGS_RUNNING" in output_str

        # 4. Forward the chosen host port -> container's Wings port (8080) and the SFTP port
        await create_fixed_port_forward(str(user.id), container_name, 8080, node_id, host_port)
        sftp_port = pick_fixed_port(node_id, list(range(20200, 20300)))
        if sftp_port:
            await create_fixed_port_forward(str(user.id), container_name, 2022, node_id, sftp_port)

        save_wings_install(container_name, domain, host_port, scheme, node_id_ptero)

        embed = create_success_embed("🐦 Wings Installed!", f"Wings daemon is set up on `{container_name}`.") if wings_ok \
            else create_warning_embed("🐦 Wings Installed (check status)", f"Setup ran on `{container_name}` but couldn't confirm the service started.")
        add_field(embed, "Domain", f"`{domain}`", False)
        add_field(embed, "Public Port", f"`{host_port}`" + (" (HTTPS via Cloudflare — set that domain's SSL mode to **Flexible**, since Wings serves plain HTTP internally here)" if scheme == "https" else " (HTTP)"), False)
        if sftp_port:
            add_field(embed, "SFTP Port", f"`{sftp_port}`", False)
        add_field(embed, "Next Step", "Blueprint installs custom Panel add-ons — see below if you'd like it.", False)
        try:
            await user.send(embed=embed)
        except discord.Forbidden:
            pass

        blueprint_view = BlueprintPromptView(user, ctx, container_name, node_id)
        await channel.send(
            content=f"{user.mention}",
            embed=create_info_embed("🧩 Install Blueprint?", "Blueprint lets you add custom Panel extensions/themes. Install it now?"),
            view=blueprint_view
        )
    except Exception as e:
        logger.error(f"Wings install failed for {container_name}: {e}")
        await channel.send(embed=create_error_embed("Wings Setup Failed", str(e)[:1500]))


class BlueprintPromptView(discord.ui.View):
    def __init__(self, user: discord.Member, ctx, container_name: str, node_id: int):
        super().__init__(timeout=300)
        self.user = user
        self.ctx = ctx
    ...
    async def _check(self, interaction: discord.Interaction) -> bool:
        if str(interaction.user.id) != str(self.user.id):
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only you can choose this."), ephemeral=True)
            return False
        return True

    @discord.ui.button(label="✅ Install Blueprint", style=discord.ButtonStyle.success)
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for child in self.children:
            child.disabled = True
        await interaction.response.edit_message(view=self)
        await interaction.followup.send(embed=create_info_embed("🧩 Installing Blueprint", "This may take a minute…"))
        try:
            output = await run_container_script(self.container_name, self.node_id, "", BLUEPRINT_INSTALL_SCRIPT, timeout=300)
            output_str = str(output) if output else ""
            if "BLUEPRINT_FAILED" in output_str:
                await interaction.followup.send(embed=create_warning_embed("Blueprint", "Blueprint's installer reported an error — you may need to run it manually inside the container. See https://blueprint.zip for docs."))
            else:
                await interaction.followup.send(embed=create_success_embed("🧩 Blueprint Installed", "Blueprint framework is ready — install extensions from the Panel's Blueprint tab."))
        except Exception as e:
            await interaction.followup.send(embed=create_error_embed("Blueprint Failed", str(e)[:1000]))
    ...
    async def no(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for child in self.children:
            child.disabled = True
        await interaction.response.edit_message(view=self)
        await interaction.followup.send(embed=create_info_embed("Skipped", "No problem — you can install Blueprint manually any time."))


@bot.command(name='wings')
async def wings_cmd(ctx, container_name: str = None):
    """Install the Wings daemon on a VPS you own (requires !pterodactyl first)."""
    if not container_name:
        await ctx.send(embed=create_error_embed("Usage", f"`{PREFIX}wings <container_name>`"))
        return
    vps = find_user_vps(ctx.author.id, container_name)
    if not vps:
        await ctx.send(embed=create_error_embed("Not Found", "You don't own a VPS with that container name."))
    ...
    install = get_pterodactyl_install(container_name)
    if not install or not install.get('panel_installed_at'):
        await ctx.send(embed=create_error_embed("Panel Required", f"Run `{PREFIX}pterodactyl {container_name}` first."))
        return
    if install.get('wings_installed_at'):
        await ctx.send(embed=create_error_embed("Already Installed", f"Wings is already installed on `{container_name}` (domain: `{install['wings_domain']}`)."))
        return
    embed = create_info_embed(
        "🐦 Wings Setup — Choose a Port",
        "Pick which public port Wings should be reachable on, then enter its domain.\n\n"
        "⚠️ Make sure this domain's DNS points at this server before starting."
    )
    view = WingsPortChoiceView(ctx.author, ctx, container_name, vps.get('node_id', 1))
    await ctx.send(embed=embed, view=view)
